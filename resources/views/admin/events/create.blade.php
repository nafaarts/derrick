        ...
        <form action="{{ route('event.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="md:w-3/4 w-full">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" class="mt-1 w-full" placeholder="Add Event title"
                        value="{{ old('name') }}">
                    @error('name')
                ...
                <div class="mb-4" x-data="{ selected: '{{ old('category') ?? 'pre' }}' }">
                    <label class="text-gray-600 capitalize">Category <span class="text-red-500">*</span></label></br>
                    <div class="flex gap-2 mt-1">
                        <label for="pre" class="w-full p-2 rounded-md flex items-center"
                            ...
                            <input type="radio" id="pre" name="category" x-on:click="selected = 'pre'"
                                @checked(old('category') == 'pre' || old('category') == '') value="pre">
                            <span class="ml-2">Pre Event</span>
                        </label>
                        <label for="post" class="w-full p-2 rounded-md flex items-center"
                            ...
                            <input type="radio" id="post" name="category" x-on:click="selected = 'post'"
                                @checked(old('category') == 'post') value="post">
            ...
            <div class="md:w-1/4 w-full">
                <div class="mb-4 mt-5">
                    <div class="flex">
                        <div class="flex-1">
                            <select name="status" id="status" class="bg-TERTIARY rounded-l-md p-2 w-full h-full">
                                <option value="1" @selected(old('status') == '1')>Save and Publish</option>
                                <option value="0" @selected(old('status') == '0')>Save Draft</option>
                            </select>
                        </div>
                        <button type="submit"
                ...
                <div class="mb-4">
                    ...
                    <label for="registration_required" class="w-full p-2 rounded-md mt-1 flex items-center">
                        <input type="checkbox" id="registration_required" name="registration_required"
                            @checked(old('registration_required')) value="1">
