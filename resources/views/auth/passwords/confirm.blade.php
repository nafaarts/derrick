...
@section('body')
    <div>
        ...
        <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900 mb-5">Confirm Password</h2>
        <hr>
    </div>
    @if ($errors->any())
    ...
    <form method="POST" class="mt-5 space-y-6" action="{{ route('password.confirm') }}">
        @csrf

        <div>
            <label for="password" class="sr-only">Password</label>
            <input id="password" name="password" type="password" autocomplete="password" required autofocus
    ...
    <small class="block text-center"><a class="text-orange-500 hover:text-orange-400"
            href="{{ route('password.request') }}">Forgot your password?</a></small>
@endsection
